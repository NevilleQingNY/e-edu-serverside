...
public final class SpecialFunction extends Object {
  ...
  */
   ...
   */
    ...
    static public double besselI0(double x) throws ArithmeticException {
     double sum = 1.0 ;
     double new_term ; 
     int counter = 1  ; 
     double threshold = 1e-3 ; 
     double convergence_ratio = 1.0 ; 
     double factorial_term = 1  ; 
     
     
     
     while(Math.abs(convergence_ratio) > threshold){
    	 new_term  = Math.pow(x,2*counter)/( Math.pow(2.0,2.0*counter)*Math.pow(factorial_term,2.0))  ; 
    	 sum = sum + new_term ; 
     counter++ ; 
     factorial_term = factorial_term*counter ;
     convergence_ratio = new_term/sum ; 
     }
